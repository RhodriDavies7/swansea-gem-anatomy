export function enhanceFilters(root){
 for(const select of root.querySelectorAll('select[data-filter]')){
  ...
  const update=()=>{trigger.textContent=select.selectedOptions[0].textContent;trigger.setAttribute('aria-label',caption+': '+trigger.textContent);trigger.classList.toggle('is-selected',!!select.value);};
  ...
  panel.append(search,list,empty);label.replaceWith(host);select.hidden=true;host.append(title,trigger,panel,select);update();
 }
}
