...
export function spotterPool(catalog,region='',{difficulty='',year='',tags=[],topics=[],tagEffect='include',topicEffect='include',models=[],modelEffect='include'}={}){
 return catalog.structures.filter(s=>s.kind!=='model-note'&&(Array.isArray(difficulty)?difficulty.includes(s.difficulty):!difficulty||s.difficulty===difficulty)&&(!year||(s.years?.length?s.years:['Year not mapped']).includes(year))&&(!region||s.regions.includes(region))&&(!tags.length||(tagEffect==='exclude'?!tags.some(t=>s.tags.includes(t)):tags.some(t=>s.tags.includes(t))))).flatMap(s=>{
